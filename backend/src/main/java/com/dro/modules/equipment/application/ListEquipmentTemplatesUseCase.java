...
public class ListEquipmentTemplatesUseCase {
    ...
    public List<EquipmentTemplateResponse> execute(Boolean activeOnly) {
        if (Boolean.TRUE.equals(activeOnly)) {
            return equipmentTemplateRepository.findByActiveTrueOrderByNameAsc().stream().map(EquipmentTemplateResponse::from).toList();
        }
        return equipmentTemplateRepository.findAllByOrderByNameAsc().stream().map(EquipmentTemplateResponse::from).toList();
    }

    public ListEquipmentTemplatesUseCase(final EquipmentTemplateRepository equipmentTemplateRepository) {
