...
public class GetDigimonInventoryUseCase {
    ...
    public List<EquipmentResponse> execute(String token, UUID digimonId) {
        ...
        var digimon = digimonRepository.findById(digimonId).orElseThrow(() -> new NotFoundException("Digimon not found"));
        ...
        return items.stream().map(EquipmentResponse::from).toList();
    }

    public GetDigimonInventoryUseCase(final EquipmentRepository equipmentRepository, final DigimonRepository digimonRepository) {
