...
public class GetEquipmentTemplateUseCase {
    ...
    public EquipmentTemplateResponse execute(String name) {
        return equipmentTemplateRepository.findByName(name).map(EquipmentTemplateResponse::from).orElseThrow(() -> new NotFoundException("Equipment template not found: " + name));
    }

    public GetEquipmentTemplateUseCase(final EquipmentTemplateRepository equipmentTemplateRepository) {
