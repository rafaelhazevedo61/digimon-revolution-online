...
public class BuyUpgradeUseCase {
    ...
    public ClanUpgradeResponse execute(String token, UUID clanId, String upgradeCode) {
        ...
        ClanUpgradeType type = upgradeTypeRepository.findById(upgradeCode)
        ...
        ClanUpgradePurchase purchase = upgradePurchaseRepository
                .findByClanIdAndUpgradeCode(clanId, upgradeCode)
                .orElseGet(() -> createEmptyPurchase(clanId, upgradeCode));
        ...
        return clanBonusService.listUpgrades(clanId).stream()
                .filter(u -> u.code().equals(upgradeCode))
                .findFirst()
                .orElseThrow(() -> new IllegalStateException("Upgrade response not found"));
    }

    private ClanUpgradePurchase createEmptyPurchase(UUID clanId, String upgradeCode) {
        return ClanUpgradePurchase.builder()
                .id(UUID.randomUUID())
                .clanId(clanId)
                .upgradeCode(upgradeCode)
                .level(0)
                .totalSpentHonorMarks(0)
                .build();
    }
