...
public class GetMyClanUseCase {
    ...
    public ClanResponse execute(String token) {
        ...
        Clan clan = clanRepository.findById(viewer.getClanId()).orElseThrow(() -> new NotFoundException("Clan not found"));
        return mapper.toResponse(clan, viewer, playerRepository.findByClanId(clan.getId()));
    }

    public GetMyClanUseCase(final ClanRepository clanRepository, final PlayerRepository playerRepository, final ClanResponseMapper mapper) {
