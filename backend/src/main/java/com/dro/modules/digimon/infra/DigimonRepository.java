...
public interface DigimonRepository extends JpaRepository<Digimon, UUID> {
    ...
    Page<Digimon> findByStatusInOrderByLevelDescExperienceDesc(@Param("statuses") List<DigimonStatus> statuses, Pageable pageable);

    @Query("SELECT d FROM Digimon d WHERE d.status IN :statuses AND d.bot = false ORDER BY " +
            ...
            "d.level DESC")
    Page<Digimon> findByStatusInOrderByGradeQualityAscLevelDesc(@Param("statuses") List<DigimonStatus> statuses, Pageable pageable);

    @Query("SELECT d FROM Digimon d WHERE d.status IN :statuses AND d.bot = false AND d.rebirthCount > :rebirthCount " +
            "ORDER BY d.rebirthCount DESC, d.level DESC")
    Page<Digimon> findByStatusInAndRebirthCountGreaterThanOrderByRebirthCountDescLevelDesc(
            @Param("statuses") List<DigimonStatus> statuses, @Param("rebirthCount") int rebirthCount, Pageable pageable);
    ...
    @Query("SELECT d FROM Digimon d WHERE d.status IN :statuses AND d.bot = false")
    List<Digimon> findByStatusInAndBotFalse(@Param("statuses") List<DigimonStatus> statuses);
