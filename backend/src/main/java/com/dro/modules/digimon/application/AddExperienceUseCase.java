...
public class AddExperienceUseCase {
    ...
    public void execute(String token, int xp) {
        ...
        Digimon digimon = repository.findById(player.getActiveDigimonId()).orElseThrow(() -> new NotFoundException("Active digimon not found"));
    ...
    public void executeForDigimon(UUID digimonId, int xp) {
        Digimon digimon = repository.findById(digimonId).orElseThrow(() -> new NotFoundException("Digimon not found"));
        digimon.gainExperience(xp);
        repository.save(digimon);
    }

    public AddExperienceUseCase(final DigimonRepository repository, final PlayerRepository playerRepository) {
