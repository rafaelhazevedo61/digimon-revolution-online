...
public class GetShopProductUseCase {
    ...
    public AdminShopProductResponse execute(String code) {
        return shopProductRepository.findById(code).map(AdminShopProductResponse::from).orElseThrow(() -> new NotFoundException("Shop product not found: " + code));
    }

    public GetShopProductUseCase(final ShopProductRepository shopProductRepository) {
