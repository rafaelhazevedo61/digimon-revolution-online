...
public class AdminShopProductController {
    ...
    @PostMapping
    public ResponseEntity<AdminShopProductResponse> create(@RequestBody @Valid CreateShopProductRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED).body(createShopProductUseCase.execute(request));
    }

    @GetMapping
    public ResponseEntity<List<AdminShopProductResponse>> list(@RequestParam(required = false) Boolean activeOnly) {
        return ResponseEntity.ok(listShopProductsUseCase.execute(activeOnly));
    }

    @GetMapping("/{code}")
    public ResponseEntity<AdminShopProductResponse> getByCode(@PathVariable String code) {
        return ResponseEntity.ok(getShopProductUseCase.execute(code));
    }

    @PutMapping("/{code}")
    public ResponseEntity<AdminShopProductResponse> update(@PathVariable String code, @RequestBody @Valid UpdateShopProductRequest request) {
        return ResponseEntity.ok(updateShopProductUseCase.execute(code, request));
    }

    @PatchMapping("/{code}/toggle-active")
    public ResponseEntity<AdminShopProductResponse> toggleActive(@PathVariable String code) {
        return ResponseEntity.ok(toggleShopProductUseCase.execute(code));
    }

    public AdminShopProductController(final CreateShopProductUseCase createShopProductUseCase, final ListShopProductsUseCase listShopProductsUseCase, final GetShopProductUseCase getShopProductUseCase, final UpdateShopProductUseCase updateShopProductUseCase, final ToggleShopProductUseCase toggleShopProductUseCase) {
        this.createShopProductUseCase = createShopProductUseCase;
        this.listShopProductsUseCase = listShopProductsUseCase;
        this.getShopProductUseCase = getShopProductUseCase;
        this.updateShopProductUseCase = updateShopProductUseCase;
        this.toggleShopProductUseCase = toggleShopProductUseCase;
    }
}
