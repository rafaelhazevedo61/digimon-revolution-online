...
public class GetAuctionHistoryUseCase {
    ...
    public java.util.List<AuctionTransactionResponse> execute(String token, int page, int size) {
        ...
        return auctionTransactionRepository.findByBuyerPlayerIdOrSellerPlayerIdOrderByCreatedAtDesc(playerId, playerId, pageable).map(transaction -> new AuctionTransactionResponse(transaction.getId(), transaction.getListing().getId(), transaction.getBuyerPlayerId().equals(playerId) ? "BUY" : "SELL", transaction.getItemDefinition().getCode(), transaction.getItemDefinition().getName(), transaction.getQuantity(), transaction.getUnitPrice(), transaction.getGrossAmount(), transaction.getFee(), transaction.getSellerNetAmount(), transaction.getCreatedAt())).getContent();
    }

    public GetAuctionHistoryUseCase(final AuctionTransactionRepository auctionTransactionRepository) {
