...
public class BuyAuctionListingUseCase {
     ...
     */
    @Transactional
    public AuctionPurchaseResponse execute(String token, UUID listingId, BuyAuctionListingRequest request) {
        ...
        AuctionTransaction transaction = AuctionTransaction.builder().id(UUID.randomUUID()).listing(listing).sellerPlayerId(listing.getSellerPlayerId()).buyerPlayerId(buyerPlayerId).itemDefinition(itemDefinition).quantity(request.quantity()).unitPrice(listing.getUnitPrice()).grossAmount(grossAmount).fee(sellerFee).sellerNetAmount(sellerNetAmount).createdAt(now).build();
     ...
     */
    private LockedDigimons lockDigimonsInStableOrder(Player buyer, Player seller, UUID buyerPlayerId) {
        ...
        UUID firstId = buyer.getActiveDigimonId().compareTo(seller.getActiveDigimonId()) < 0 ? buyer.getActiveDigimonId() : seller.getActiveDigimonId();
        UUID secondId = firstId.equals(buyer.getActiveDigimonId()) ? seller.getActiveDigimonId() : buyer.getActiveDigimonId();
    ...
    private Digimon findLockedDigimon(UUID digimonId, Player buyer, Player seller, UUID buyerPlayerId) {
        Digimon digimon = digimonRepository.findByIdForUpdate(digimonId).orElseThrow(() -> new ConflictException("Active Digimon not found"));
        boolean belongsToBuyer = buyer.getId().equals(buyerPlayerId) && digimon.getPlayerId().equals(buyer.getId());
    ...
    private void saveBuyerInventory(InventoryItem inventoryItem, Digimon buyerDigimon, ItemDefinition itemDefinition, int newQuantity) {
    ...
    public BuyAuctionListingUseCase(final PlayerRepository playerRepository, final DigimonRepository digimonRepository, final InventoryRepository inventoryRepository, final AuctionListingRepository auctionListingRepository, final AuctionTransactionRepository auctionTransactionRepository, final AuctionMailNotificationService auctionMailNotificationService, final TransactionAuditPublisher transactionAuditPublisher) {
        ...
        this.auctionMailNotificationService = auctionMailNotificationService;
