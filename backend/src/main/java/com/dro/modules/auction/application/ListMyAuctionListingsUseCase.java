...
public class ListMyAuctionListingsUseCase {
    ...
    public AuctionListingPageResponse execute(String token, int page, int size) {
        ...
        Page<com.dro.modules.auction.domain.AuctionListing> listings = auctionListingRepository.findSellerListings(playerId, pageable);
        var player = playerRepository.findById(playerId).orElse(null);
        return AuctionListingMapper.toPageResponse(listings, player == null ? java.util.List.of() : java.util.List.of(player));
    }

    public ListMyAuctionListingsUseCase(final AuctionListingRepository auctionListingRepository, final PlayerRepository playerRepository) {
