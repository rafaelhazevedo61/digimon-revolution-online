...
public class CreateAuctionListingUseCase {
    ...
    public AuctionListingResponse execute(String token, CreateAuctionListingRequest request) {
        ...
        InventoryItem inventoryItem = inventoryRepository.findByDigimonIdAndItemDefinitionIdForUpdate(sellerDigimon.getId(), itemDefinition.getId()).orElseThrow(() -> new NotFoundException("Item not found in active Digimon inventory"));
        ...
        AuctionListing listing = AuctionListing.builder().id(UUID.randomUUID()).sellerPlayerId(playerId).sellerDigimonId(sellerDigimon.getId()).itemDefinition(itemDefinition).quantity(request.quantity()).remainingQuantity(request.quantity()).unitPrice(request.unitPrice()).listingFee(AuctionRules.LISTING_FEE).sellerFeeRateBps(sellerFeeRateBps).status(AuctionListingStatus.ACTIVE).createdAt(now).expiresAt(AuctionRules.expirationAt(now, request.durationHours())).updatedAt(now).build();
