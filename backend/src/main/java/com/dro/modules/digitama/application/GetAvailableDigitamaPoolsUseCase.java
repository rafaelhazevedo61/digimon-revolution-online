...
public class GetAvailableDigitamaPoolsUseCase {
    ...
    private AvailableDigitamaPoolResponse toResponse(DigitamaPool pool) {
        List<AvailableDigitamaEntryResponse> entries = pool.getEntries().stream().filter(DigitamaPoolEntry::isActive).sorted(Comparator.comparingInt(DigitamaPoolEntry::getWeight).reversed()).map(this::toEntryResponse).toList();
        return new AvailableDigitamaPoolResponse(pool.getCode(), pool.getName(), pool.getDescription(), entries);
    }
    ...
    public GetAvailableDigitamaPoolsUseCase(final DigitamaPoolRepository digitamaPoolRepository) {
