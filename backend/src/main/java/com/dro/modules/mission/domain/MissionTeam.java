...
public class MissionTeam {
    ...
    @Column(name = "digimon_2_id", nullable = false)
    private UUID digimon2Id;

    @Column(name = "digimon_3_id", nullable = false)
    ...
    public MissionTeam(UUID playerId, String name, List<UUID> digimonIds, UUID captainDigimonId) {
        if (digimonIds == null || digimonIds.size() != 3) {
            throw new IllegalArgumentException("Um time precisa ter exatamente 3 Digimons");
        }
        this.playerId = playerId;
        this.name = name;
        this.digimon1Id = digimonIds.get(0);
        this.digimon2Id = digimonIds.get(1);
        this.digimon3Id = digimonIds.get(2);
    ...
    public List<UUID> getDigimonIds() {
        return List.of(digimon1Id, digimon2Id, digimon3Id);
    }
}
