...
public class GetAvailableMissionsUseCase {
    ...
    public List<MissionResponse> execute(String token) {
        ...
        return missionDefinitionRepository.findByActiveTrue().stream().filter(m -> level >= m.getRequiredLevel()).filter(m -> stage.ordinal() >= m.getRequiredStage().ordinal()).map(m -> new MissionResponse(m.getId(), m.getName(), m.getDescription(), m.getArea().name(), m.getRequiredLevel(), m.getBaseXp(), m.getBaseBits(), m.getEnergyCost(), m.getDurationSeconds())).toList();
    }
