...
public class ListMissionChestOptionsUseCase {
    ...
    public List<MissionChestOptionResponse> execute() {
        return chestDefinitionRepository.findByActiveTrueOrderByNameAsc().stream().map(MissionChestOptionResponse::from).toList();
    }

    public ListMissionChestOptionsUseCase(final ChestDefinitionRepository chestDefinitionRepository) {
