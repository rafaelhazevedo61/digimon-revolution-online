...
public class GetIncubationUseCase {
    ...
    public IncubationResponse execute(String token) {
        ...
        Incubation incubation = incubationRepository.findByPlayerIdAndStatus(playerId, IncubationStatus.IN_PROGRESS).orElse(null);
        if (incubation == null) {
            return null;
        }
        long remaining = Duration.between(LocalDateTime.now(), incubation.getFinishAt()).getSeconds();
        if (remaining < 0) {
            remaining = 0;
        }
        return new IncubationResponse(incubation.getDigitamaType(), incubation.getIncubatorType(), incubation.getStatus(), incubation.getStartedAt(), incubation.getFinishAt(), remaining);
    }

    public GetIncubationUseCase(final IncubationRepository incubationRepository) {
