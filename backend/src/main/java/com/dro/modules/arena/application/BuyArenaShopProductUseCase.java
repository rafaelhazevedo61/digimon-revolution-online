...
public class BuyArenaShopProductUseCase {
    ...
    public BuyArenaShopResponse execute(String token, BuyArenaShopRequest request) {
        ...
        ArenaShopProduct product = arenaShopProductRepository.findById(request.productCode())
                .filter(ArenaShopProduct::isActive)
                .orElseThrow(() -> new NotFoundException("Arena shop product not found: " + request.productCode()));
        ...
        return new BuyArenaShopResponse(
                product.getCode(),
                product.getName(),
                product.getItemType(),
