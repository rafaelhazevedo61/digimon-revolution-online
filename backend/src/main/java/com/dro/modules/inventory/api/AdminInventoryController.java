...
public class AdminInventoryController {
    ...
    public ResponseEntity<List<Map<String, String>>> listItemDefinitions() {
        ...
        List<Map<String, String>> result = items.stream().map(i -> Map.of("code", i.getCode(), "name", i.getName(), "category", i.getCategory())).toList();
        return ResponseEntity.ok(result);
    }

    public AdminInventoryController(final AddItemUseCase addItemUseCase, final ItemDefinitionRepository itemDefinitionRepository, final AdminAuditService adminAuditService) {
