...
public class GetAvailableEvolutionLinesUseCase {
    ...
    public List<AvailableEvolutionLineResponse> execute() {
        return evolutionLineRepository.findByActiveTrueAndContentActiveTrue().stream().map(this::toResponse).toList();
    }

    private AvailableEvolutionLineResponse toResponse(EvolutionLine line) {
        List<AvailableEvolutionLineStepResponse> steps = line.getSteps().stream().sorted(Comparator.comparingInt(EvolutionLineStep::getStepOrder)).map(this::toStepResponse).toList();
        return new AvailableEvolutionLineResponse(line.getCode(), line.getName(), line.getDescription(), steps);
    }
