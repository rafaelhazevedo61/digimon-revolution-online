...
public class GetEvolutionLinesUseCase {
    ...
    public EvolutionLinePageResponse execute(
            String code,
            String name,
            Boolean active,
            Pageable pageable
    ) {
        Page<EvolutionLineResponse> lines = evolutionLineRepository.findAll(
                        EvolutionLineSpecifications.withFilters(code, name, active),
                        pageable
                )
                .map(this::toResponse);
    ...
    private EvolutionLineResponse toResponse(EvolutionLine line) {
        List<EvolutionLineStepResponse> steps = line.getSteps()
                .stream()
                .sorted(Comparator.comparing(EvolutionLineStep::getStepOrder))
                .map(this::toStepResponse)
                .toList();

        return new EvolutionLineResponse(
                line.getId(),
                line.getCode(),
                line.getName(),
                line.isActive(),
    ...
    private EvolutionLineStepResponse toStepResponse(EvolutionLineStep step) {
        List<EvolutionLineStepMaterialResponse> materials = step.getMaterials()
                .stream()
                .map(this::toMaterialResponse)
                .toList();

        return new EvolutionLineStepResponse(
                step.getStepOrder(),
                step.getDigimonInfo().getId(),
                step.getDigimonInfo().getName(),
                step.getDigimonInfo().getStage().name(),
                step.getRequiredLevel(),
                materials
        );
    }

    private EvolutionLineStepMaterialResponse toMaterialResponse(EvolutionStepMaterial material) {
        ItemDefinition itemDefinition = itemDefinitionRepository.findByCode(material.getMaterialCode())
                .orElse(null);

        if (itemDefinition == null) {
            return new EvolutionLineStepMaterialResponse(
                    null,
