...
public class GetAvailableBossesUseCase {
    ...
    public List<BossDefinitionResponse> execute(String token) {
        ...
        List<BossDefinitionEntity> bosses = bossDefinitionRepository.findAllActive().stream().filter(boss -> boss.getBossType() != BossType.CLAN && boss.getBossType() != BossType.WORLD).toList();
        Set<Long> todaysDailyBossIds = getTodaysDailyBossIds(bosses);
        return bosses.stream().filter(boss -> boss.getBossType() != BossType.DAILY || todaysDailyBossIds.contains(boss.getId())).map(boss -> {
            boolean meetsRequirements = digimon.getStage().ordinal() >= boss.getRequiredStage().ordinal() && digimon.getLevel() >= boss.getRequiredLevel() && digimon.getRebirthCount() >= boss.getRequiredRebirths();
    ...
    private Set<Long> getTodaysDailyBossIds(List<BossDefinitionEntity> allBosses) {
        ...
        Map<String, List<BossDefinitionEntity>> dailyByStage = allBosses.stream().filter(b -> b.getBossType() == BossType.DAILY).sorted(Comparator.comparingLong(BossDefinitionEntity::getId)).collect(Collectors.groupingBy(b -> b.getRequiredStage().name(), LinkedHashMap::new, Collectors.toList()));
    ...
    public GetAvailableBossesUseCase(final BossDefinitionRepository bossDefinitionRepository, final BossAttemptRepository bossAttemptRepository, final PlayerRepository playerRepository, final DigimonRepository digimonRepository, final EquipmentRepository equipmentRepository) {
