...
public class GetBossCooldownsUseCase {
    ...
    public List<BossCooldownResponse> execute(String token) {
        ...
        return bosses.stream().map(boss -> {
            ...
            return new BossCooldownResponse(boss.getCode(), boss.getName(), boss.getBossType().name(), available, cooldownRemaining);
        }).toList();
    }

    public GetBossCooldownsUseCase(final BossDefinitionRepository bossDefinitionRepository, final BossAttemptRepository bossAttemptRepository) {
