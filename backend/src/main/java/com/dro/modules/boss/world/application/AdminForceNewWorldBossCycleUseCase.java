...
public class AdminForceNewWorldBossCycleUseCase {
    ...
    public WorldBossInstance execute() {
        ...
        WorldBossInstance current = worldBossInstanceRepository.findFirstByBossDateOrderByCreatedAtDesc(today).orElseThrow(() -> new ConflictException("Não existe uma instância do Boss Mundial hoje para substituir."));
        if (current.getStatus() != WorldBossStatus.DEFEATED) {
            throw new ConflictException("O novo ciclo só pode ser aberto depois que o Boss Mundial atual for derrotado.");
        }
        try {
            return worldBossInstanceFactory.create(today, current.getCycleNumber() + 1);
        } catch (DataIntegrityViolationException exception) {
            throw new ConflictException("Já existe um novo ciclo do Boss Mundial para hoje.");
        }
    }

    public AdminForceNewWorldBossCycleUseCase(final WorldBossInstanceRepository worldBossInstanceRepository, final WorldBossInstanceFactory worldBossInstanceFactory) {
