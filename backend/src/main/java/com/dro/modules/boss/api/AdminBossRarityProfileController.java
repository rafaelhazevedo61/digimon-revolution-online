...
public class AdminBossRarityProfileController {
    ...
    public ResponseEntity<AdminEquipmentRarityProfileResponse> update(@PathVariable String profileKey, @RequestBody @Valid UpdateEquipmentRarityProfileRequest request) {
        return ResponseEntity.ok(profileService.updateBossProfile(profileKey, request));
    }

    public AdminBossRarityProfileController(final EquipmentRarityProfileService profileService) {
