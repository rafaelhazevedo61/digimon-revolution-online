...
async function missionShowLootModal(missionId) {
  const mission = missionState.missions.find(item => String(item.id) === String(missionId));
  if (!mission) return;
  ...
  overlay.className = "fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 py-6";
  overlay.innerHTML = `<div class="card w-full max-w-6xl max-h-[92vh] overflow-y-auto" onclick="event.stopPropagation()"><p class="text-slate-400">Carregando configuração completa do loot...</p></div>`;
  overlay.onclick = event => { if (event.target === overlay) overlay.remove(); };
  document.body.appendChild(overlay);

  let lootTable = null;
  let loadError = null;
  if (mission.chestLootTableCode) {
    try {
      lootTable = await apiGet(`/admin/loot-tables/${encodeURIComponent(mission.chestLootTableCode)}`);
    } catch (error) {
      loadError = error;
    }
  }
  if (!document.getElementById("mission-loot-modal")) return;

  const rewards = Array.isArray(mission.rewards) ? mission.rewards : [];
  const chances = Array.isArray(lootTable?.rarityWeights) && lootTable.rarityWeights.length
    ? lootTable.rarityWeights.map(value => ({ rarity: value.rarity, chance: value.weight }))
    : (Array.isArray(mission.lootChances) ? mission.lootChances : []);
  const items = Array.isArray(lootTable?.entries) && lootTable.entries.length
    ? lootTable.entries
    : (Array.isArray(mission.lootItems) ? mission.lootItems : []);
  const issues = [];
  ...
  if (loadError) issues.push(`Não foi possível carregar a Loot Table em produção: ${loadError.message}`);
  if (mission.chestLootTableCode && !lootTable && !loadError) issues.push("A Loot Table associada não foi encontrada.");
  ...
  const entryName = value => value.itemName || value.itemType || value.itemCode || value.materialCode || "-";
  const entryMin = value => value.minQuantity ?? value.quantity ?? 0;
  const entryMax = value => value.maxQuantity ?? value.quantity ?? 0;
  const root = document.getElementById("mission-loot-modal");
  root.innerHTML = `<div class="card w-full max-w-6xl max-h-[92vh] overflow-y-auto" onclick="event.stopPropagation()">
    <div class="flex items-start justify-between gap-4 border-b border-slate-800 pb-4 mb-5"><div><p class="text-xs uppercase tracking-wider text-cyan-400 font-bold">Diagnóstico administrativo</p><h2 class="text-2xl font-bold text-slate-100 mt-1">Loot de ${escapeHtml(mission.name)}</h2><p class="text-sm text-slate-400 mt-1">${escapeHtml(mission.id)} · ${escapeHtml(AREA_LABELS[mission.area] || mission.area || "Área não informada")}</p></div><button class="text-slate-400 hover:text-white text-2xl" aria-label="Fechar" onclick="document.getElementById('mission-loot-modal')?.remove()">&times;</button></div>
    ${issues.length ? `<div class="rounded-lg border border-amber-700 bg-amber-950/30 p-4 mb-5"><h3 class="font-bold text-amber-300">Atenção na configuração</h3><ul class="mt-2 space-y-1 text-sm text-amber-100">${issues.map(issue => `<li>• ${escapeHtml(issue)}</li>`).join("")}</ul></div>` : `<div class="rounded-lg border border-emerald-700 bg-emerald-950/30 p-4 mb-5 text-sm text-emerald-200">Configuração carregada diretamente da Loot Table.</div>`}
    <div class="grid gap-4 md:grid-cols-4 mb-6"><div class="card-sm"><p class="text-xs text-slate-500">Baú da Área</p><p class="font-semibold text-slate-100 mt-1">${escapeHtml(mission.chestName || "Não configurado")}</p><p class="text-xs text-cyan-300 font-mono mt-1">${escapeHtml(mission.chestCode || "-")}</p></div><div class="card-sm"><p class="text-xs text-slate-500">Loot Table</p><p class="font-semibold text-slate-100 mt-1">${escapeHtml(lootTable?.name || mission.chestLootTableName || "Não configurada")}</p><p class="text-xs text-cyan-300 font-mono mt-1">${escapeHtml(mission.chestLootTableCode || "-")}</p></div><div class="card-sm"><p class="text-xs text-slate-500">Itens</p><p class="font-semibold text-slate-100 mt-1">${items.length}</p></div><div class="card-sm"><p class="text-xs text-slate-500">Faixa de sorteio</p><p class="font-semibold text-slate-100 mt-1">${lootTable ? `${lootTable.minItems ?? 0}–${lootTable.maxItems ?? 0}` : "-"}</p></div></div>
    ...
    <h3 class="text-lg font-bold text-violet-300 mt-6 mb-2">Pesos de raridade</h3>${rows(chances, [{label:"Raridade",render:v=>`<span class="badge">${escapeHtml(v.rarity || "-")}</span>`},{label:"Peso",render:v=>`<span class="text-amber-300 font-semibold">${v.chance ?? 0}</span>`}], "Nenhum peso configurado.")}
    <h3 class="text-lg font-bold text-cyan-300 mt-6 mb-2">Itens possíveis</h3>${rows(items, [{label:"Item",render:v=>`<span class="font-semibold text-slate-100">${escapeHtml(entryName(v))}</span><span class="block text-xs font-mono text-slate-500">${escapeHtml(v.itemCode || v.materialCode || v.itemType || "-")}</span>`},{label:"Raridade",render:v=>escapeHtml(v.rarity || "-")},{label:"Peso",render:v=>`${v.weight ?? "-"}`},{label:"Quantidade",render:v=>`${entryMin(v)}–${entryMax(v)}`}], "Nenhum item configurado.")}
