    ...
    <div class="page-container pb-24">
      <div class="flex items-center justify-between mb-4 px-1">
        <div>
          ...
          <p class="text-xs text-slate-400 mt-1">Compre e venda itens e equipamentos com outros jogadores.</p>
        </div>
    ...
    <div class="card space-y-3">
      <div class="grid grid-cols-2 gap-2">
        ...
        <select id="auction-category" class="input" aria-label="Filtrar por categoria">
          ...
          ${auctionFilterOption("EQUIPMENT", "Equipamentos", auctionState.category)}
      ...
      <div class="grid grid-cols-2 gap-2 mt-3">
        ...
        <button class="btn-secondary w-full" onclick="auctionShowCreateForm()">+ Publicar anúncio</button>
      </div>
    ...
    <div class="card min-h-[8.5rem] p-4">
      <div class="grid grid-cols-[3.5rem_minmax(0,1fr)] sm:grid-cols-[3.5rem_minmax(0,1fr)_9.5rem] gap-x-3 gap-y-3 items-center h-full">
        ...
        <div class="min-w-0 self-stretch flex flex-col justify-center">
          ...
          ${auctionEquipmentDetails(listing.equipment)}
...
function auctionEquipmentDetails(equipment) {
  if (!equipment) return "";
  const slot = { WEAPON: "Arma", ARMOR: "Armadura", ACCESSORY: "Acessório" }[equipment.slot] || equipment.slot;
  return `<div class="text-xs text-cyan-200 mt-1 space-y-1">
    <p>${escapeHtml(slot || "")} · ${escapeHtml(auctionRarityLabel(equipment.rarity))} · ${escapeHtml(equipment.setCode || "")}</p>
    <p>T${Number(equipment.tier)} · +${Number(equipment.refinementLevel)} · Ascensão ${Number(equipment.ascensionLevel)}</p>
    <p>HP +${Number(equipment.effectiveBonusHp)} · ATK +${Number(equipment.effectiveBonusAttack)} · DEF +${Number(equipment.effectiveBonusDefense)}</p>
    <p class="text-slate-500">Base: HP ${Number(equipment.bonusHp)} · ATK ${Number(equipment.bonusAttack)} · DEF ${Number(equipment.bonusDefense)}</p>
  </div>`;
}

function auctionIconMarkup(listing) {
  if (listing.equipment) {
    const icon = { WEAPON: "⚔️", ARMOR: "🛡️", ACCESSORY: "💍" }[listing.equipment.slot] || "⚔️";
    return `<span class="text-3xl" aria-hidden="true">${icon}</span>`;
  }
...
async function auctionShowCreateForm(type = "ITEM") {
  auctionState.createType = type === "EQUIPMENT" ? "EQUIPMENT" : "ITEM";
  const content = document.getElementById("auction-content");
  content.innerHTML = `<div class="card text-center text-slate-400">Carregando inventário...</div>`;
  try {
    const isEquipment = auctionState.createType === "EQUIPMENT";
    auctionState.inventory = await apiGet(isEquipment ? "/equipment/inventory" : "/inventory") || [];
    const eligible = isEquipment
      ? auctionState.inventory.filter(e => !e.equipped && !e.locked).map(e => ({
          quantity: 1, equipment: e, itemDefinition: { id: e.id, name: e.name, code: e.setCode, itemType: e.slot }
        }))
      : auctionState.inventory.filter(item =>
    ...
    content.innerHTML = `
      <div class="card space-y-4">
        <div class="flex items-center justify-between">
          <div>
            <h3 class="font-bold">Publicar anúncio</h3>
        ...
        <div class="grid grid-cols-2 gap-2">
          <button type="button" class="${isEquipment ? "btn-secondary" : "btn-primary"}" onclick="auctionShowCreateForm('ITEM')">Itens</button>
          <button type="button" class="${isEquipment ? "btn-primary" : "btn-secondary"}" onclick="auctionShowCreateForm('EQUIPMENT')">Equipamentos</button>
        </div>
        ${eligible.length ? `
        ...
        ` : `<p class="text-sm text-slate-400">Nenhum item disponível nesta categoria. Equipamentos equipados ou bloqueados não podem ser publicados.</p>`}
      </div>
    `;
...
function auctionSelectCreateItem(itemDefinitionId) {
  auctionState.createSelectedItemId = String(itemDefinitionId);
  const item = auctionState.createEligible.find(entry => String(entry.itemDefinition.id) === auctionState.createSelectedItemId);
  ...
  if (quantityInput) {
    ...
    quantityInput.closest("label").hidden = Boolean(item.equipment);
    if (item.equipment) quantityInput.value = 1;
    if (Number(quantityInput.value) > item.quantity) quantityInput.value = item.quantity;
  }
  if (selectedContainer) {
    selectedContainer.innerHTML = `<div class="card-sm flex items-center justify-between gap-3 border-cyan-800 bg-cyan-950/30"><div class="min-w-0"><p class="font-semibold text-cyan-200 truncate">${escapeHtml(item.itemDefinition.name)}</p>${auctionEquipmentDetails(item.equipment)}<p class="text-xs text-slate-400 mt-1">${Number(item.quantity).toLocaleString("pt-BR")} disponível(is)</p></div><span class="text-xs text-cyan-300 shrink-0">Selecionado</span></div>`;
  }
}
...
function auctionFilteredCreateItems() {
  ...
  return auctionState.createEligible.filter(item => {
    ...
    return [definition.name, definition.code, definition.itemType, item.equipment?.rarity, item.equipment?.tier]
      .filter(Boolean)
...
function auctionRenderCreateItemModalResults() {
  ...
  const results = pageItems.length
    ? pageItems.map(item => `<button type="button" class="card-sm w-full text-left hover:border-cyan-600 ${String(item.itemDefinition.id) === String(auctionState.createSelectedItemId) ? "border-cyan-500 bg-cyan-950/30" : ""}" data-auction-create-item-id="${item.itemDefinition.id}"><div class="flex items-center justify-between gap-3"><span class="min-w-0"><span class="block text-cyan-300 font-medium truncate">${escapeHtml(item.itemDefinition.name)}</span>${auctionEquipmentDetails(item.equipment)}<span class="block text-xs text-slate-500">${escapeHtml(item.itemDefinition.code || item.itemDefinition.itemType || "Item")} · ${Number(item.quantity).toLocaleString("pt-BR")} disponível(is)</span></span><span class="text-xs text-slate-400 shrink-0">Selecionar</span></div></button>`).join("")
...
async function auctionCreate(event) {
  ...
  const selectedId = document.getElementById("auction-create-item").value;
  const selected = auctionState.createEligible.find(e => String(e.itemDefinition.id) === selectedId);
  if (!selected) {
  ...
  try {
    const asset = selected.equipment
      ? { listingType: "EQUIPMENT", equipmentId: selected.equipment.id, quantity: 1 }
      : { listingType: "ITEM", itemDefinitionId: Number(selected.itemDefinition.id), quantity };
    await apiPost("/auction/listings", { ...asset, unitPrice, durationHours });
...
function auctionOpenBuyModal(listing) {
  ...
  overlay.dataset.listingType = listing.listingType || "ITEM";
  ...
  overlay.innerHTML = `
    <div class="bg-slate-900 border border-cyan-900 rounded-xl max-w-md w-full p-5 shadow-2xl">
      ...
      <form onsubmit="auctionConfirmBuy(event)" class="space-y-4">
        <label class="block text-sm text-slate-300" ${listing.listingType === "EQUIPMENT" ? "hidden" : ""}>
          Quantidade
...
function auctionUpdateBuyModalSummary() {
  ...
  const isValid = Number.isInteger(quantity) && quantity >= 1 && quantity <= maxQuantity
    && (modal.dataset.listingType !== "EQUIPMENT" || quantity === 1);
...
function auctionOpenCancelModal(listing) {
  ...
  overlay.innerHTML = `
    <div class="bg-slate-900 border border-amber-900 rounded-xl max-w-md w-full p-5 shadow-2xl">
      ...
      <p class="text-xs text-slate-400 mb-4">O conteúdo do anúncio será devolvido ao seu inventário. A taxa de publicação não é reembolsada.</p>
      <div class="grid grid-cols-2 gap-2">
  ...
  const rows = (transactions || []).map(transaction => `
    <div class="card flex items-center justify-between gap-3">
      <div>
        ...
        ${auctionEquipmentDetails(transaction.equipment)}
