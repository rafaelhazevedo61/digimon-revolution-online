...
function dexRender() {
  ...
  let html = `
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3" aria-label="Digimons disponíveis">
      ${dexEntries.map(d => `
        <button type="button" class="card-sm group text-left p-2 cursor-pointer transition-all hover:border-cyan-500 hover:bg-slate-800/80 focus:outline-none focus:ring-2 focus:ring-cyan-400" onclick="dexShowDetail(${d.id})" aria-label="Ver detalhes de ${escapeAttr(d.name)}">
          ...
          <p class="font-bold text-xs sm:text-sm truncate mt-2" title="${escapeAttr(d.name)}">${escapeHtml(d.name)}</p>
        </button>
      `).join("")}
    </div>
  `;
